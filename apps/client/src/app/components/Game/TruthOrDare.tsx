...
export function TruthOrDare({ session }: { session: Session }) {
  ...
  const chooseRandomQuestion = (type: string) => {
    const questions = session.questions.filter(q => q.content === type)
    const ran_index = Math.floor(Math.random() * questions.length)
    setChosenQuestion(questions[ran_index])
  }

  return (
    <div className={classes.main}>
      {chosenPlayer ? <>
        <h1 className={classes.heading}>The chosen player is</h1>
        <div className={`rounded shadow ${classes.card}`}>
          <span>
            {chosenPlayer.name}
          </span>
        </div>
        {chosenPlayer.id === localStorage.getItem('id') && !chosenQuestion ? <div className={classes.action}>
          <Button
            onClick={() => chooseRandomQuestion("Truth")}
            fullWidth
            variant="contained"
            color="success"
          ...
          <Button
            onClick={() => chooseRandomQuestion("Dare")}
            fullWidth
            variant="contained"
            color="error"
            className={classes.button}
